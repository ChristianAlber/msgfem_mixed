...
%         basis{j}.par_ptilde = basis{j}.par_p - mean(basis{j}.par_p(ismember(msh.Emg{j}.elements, msh.Par{j}.elements)));
        ...
        % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
        % Solve the local eigenvalues problems for the velocities (WORKING) %
        % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
        %\
...
%         toc
%         disp(['running time (building the nullspace): ',num2str(toc)])
...
%         tic
        [Vtmp{j}(FREE,:), D] = eigs(MA(FREE, FREE), MB(FREE, FREE), nloc, 'sm', 'Tolerance', 1e-15);
%         toc
%         disp(['running time (solving evp): ',num2str(toc)])

        %\
        
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
        % Solve the local eigenvalues problems without computing nullspaces (not working) %
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
...
% 
% 
%         FREE1 = setdiff(1:msh.Emg{j}.totdof, msh.Emg{j}.gBlfaces );
% 
%         vec_tmp = zeros(2 * msh.Emg{j}.totdof,1);
%         vec_tmp(ndofv+1:ndofp+ndofv) = 1;
% 
%         MA = [Klocal{j} Klocal{j}'; Klocal{j} sparse(msh.Emg{j}.totdof, msh.Emg{j}.totdof)];
%         MA = [MA vec_tmp; vec_tmp' 0];
%         Ctmp = [Klocal_Omg{j}(1:ndofv,1:ndofv) Zpv';Zpv Zpp];
%         MB = [Ctmp sparse(msh.Emg{j}.totdof, msh.Emg{j}.totdof); sparse(msh.Emg{j}.totdof, msh.Emg{j}.totdof) sparse(msh.Emg{j}.totdof, msh.Emg{j}.totdof)];
%         MB = [MB zeros(2 * msh.Emg{j}.totdof,1); zeros(2 * msh.Emg{j}.totdof,1)' 0];
%         FREE2 = msh.Emg{j}.totdof + setdiff(1:msh.Emg{j}.totdof, msh.Emg{j}.Blfaces);
%         FREE = union(FREE1, FREE2);
%         FREE = union(FREE, 2*msh.Emg{j}.totdof+1);
% 
%          % Allocate space for Eigenfunctions
%         Vtmp{j} = zeros(2 * msh.Emg{j}.totdof+1,nloc);
% 
%         % Solve generalized Eigenproblem using eigs
% %         tic
%         [Vtmp{j}(FREE,:), D] = eigs(MA(FREE, FREE), MB(FREE, FREE), nloc, 'sm', 'Tolerance', 1e-15);
% %         toc
% %         disp(['running time (solving very constrained evp): ',num2str(toc)])
...
%         Ptmp2{j} = zeros(ndofp,nloc);
        ...
        for i = 1 : nlocp
            ...
            tmp_sol(FREE(1:end-1)) = K_tmp(FREE(1:end-1),FREE(1:end-1)) \ ftmp(FREE(1:end-1));
...
%             Ptmp{j}(msh.Par{j}.lelements,i) = sol_tmp(msh.Par{j}.lelements);     
        ...
        for i = 1 : nlocp
            tmp_sol = zeros(msh.Emg{j}.totdof, 1);
            % tmp_sol2 = zeros(msh.Emg{j}.totdof, 1);
            ...
            K_tmp2 = Klocal_Par{j};
            tmp_sol(FREE(1:end-1)) = K_tmp2(FREE(1:end-1),FREE(1:end-1)) \ ftmp(FREE(1:end-1));

            % % % Second variant:
            % FREE = setdiff(1:msh.Emg{j}.totdof, [con_z msh.Par{j}.Blfaces']);
            % tmp_sol(FREE) = K_en \ ftmp(FREE);

            % disp(abs(mean(tmp_sol - tmp_sol2)))
        ...
        % The following code leaves out one of of the coarse pressure basis
        % functions in order for the coarse system to be well defined.
        if j == msh.numDomain
            basis{j}.PRT = [];
        else
            basis{j}.PRT = msh.Emg{j}.Res_p * ones(msh.Emg{j}.nel,1);
