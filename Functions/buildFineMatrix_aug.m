function [MPVA, MPV, MPV0, Aug] = buildFineMatrix_aug(msh, coeffs, gamma)
%BUILDFINEMATRIX_aug builds the fine matrix for the Darcy problem, where
%the augmented formulation is used.
%   gamma: parameter for augmented formulation
...
% Get the local matrices corresponding to the Augmented form
Aug = augmentedForm(msh.elems2faces',signs, msh.hx, gamma);
% Add the local augmented matrices to the usual local matrices
MPV(1:4,1:4,:) = MPV(1:4,1:4,:) + Aug;
%test
% for i = 1:msh.nelem
%     for j = 1:4
